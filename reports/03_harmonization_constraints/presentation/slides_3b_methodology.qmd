...
```{mermaid}
flowchart TB
    pairs["1,598 Question Pairs\n(CPS-ACS, FoodAPS-ACS)"]

    subgraph rating["Rating Tier — Fast Models"]
        direction LR
        r1["GPT-4o-mini"]
        r2["Claude Haiku 4.5"]
        r3["Gemini 2 Flash"]
    ...
    subgraph arb["Arbitration Tier — Flagship Models"]
        direction LR
        a1["GPT-5.2"]
        a2["Claude Opus 4.5"]
        a3["Gemini 3 Pro"]
    ...
    rollup["Question-Level Rollup\n1,598 pairs → 380 questions\nBest-match selection"]

    output["Expert Review Table\n380 questions with classifications,\nconfidence scores, barrier codes,\ntriage routing"]
...
::: {.notes}
Key: Both tiers process ALL pairs independently. This is parallel evaluation at two cost/quality points, not a filter-funnel. Enables full behavioral comparison between tiers.
:::

---

## Validation at Each Stage {.smaller}

| Stage | What Happens | Validation | Key Finding |
|-------|-------------|------------|-------------|
| **Input** | 1,598 question pairs (CPS-ACS, FoodAPS-ACS) | — | — |
| **Rating** | 3 fast models independently classify all pairs (F1/F2/F3 + barrier) | Inter-rater: κ=0.611 (substantial) | Independent architectures converge — task well-defined |
| **Arbitration** | 3 flagship models independently re-evaluate ALL pairs | Inter-arbitrator: κ=0.843 (almost perfect) | Deeper reasoning → higher convergence |
| **Rollup** | 1,598 pairs → 380 questions; best-match + Borda/Entropy scoring | Triage quadrant assignment | 76% auto-process, 24% expert review |
| **Output** | Expert review table with classifications, confidence, reasoning | — | 380 questions, pre-analyzed, ready for validation |

::: {.notes}
Pipeline A (diagram): how data flows through models. Pipeline B (this table): what we measured at each stage and what it means. Validation runs ON outputs of processing — separate concerns.
:::
...
- **Reusable methodology** for harmonization analysis across additional survey pairs
:::
...
**Impact:** Validated methodology for AI-assisted survey harmonization — enabling systematic bridge variable identification across the federal survey landscape
