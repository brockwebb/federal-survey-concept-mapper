...
**Completion note 2026-06-12: v2 AHS id inflation corrected (text unit).** The AHS best-tier split script's denominator assert failed on WORK: 89 unique question texts vs 92 ids in the candidate set, 4 texts carrying more than one survey_q_id. Same defect class as the 2026-02 v1 correction, recurring in the v2 AHS rollup, which had collapsed question-level metrics on `survey_q_id`. `stage3_ahs_rollup.py` was revised to collapse on unique question text everywhere and rerun on WORK. Corrected values, id-based (retired) to text unit: entered pairing 744 to 736; with candidate 92 to 89; reach 12.37% to 12.09%; all-F3 bucket 389 to 388; no-shared-subtopic bucket 263 to 259; best tier now reported at question level, F1 47 and F2 42. Topic reach shifted accordingly (Demographic 65.91% to 64.29%, Housing 11.33% to 11.11%). Pair-level figures (1626 total, 130 candidates, 71 F1 / 59 F2 / 1496 F3 pairs) are unaffected. Id counts are retained under `id_diagnostics` in `ahs_candidate_summary.json`, labeled do-not-cite. Downstream surfaces updated to the text unit: `v2/report/v2_report_numbers_collected.json`, `v2/report/v2_results_report.qmd`, `v2/report/three_survey_harmonization_summary.qmd`. The assert that caught this exists because of the 2026-02 lesson; it worked.
