...
```mermaid
flowchart TB
    Start([Raw Survey Questions<br/>6,987 questions from 48 surveys])
...
flowchart TB
    ...
    Step1[claude-haiku-4-5<br/>6 parallel workers<br/>~1.5 hours]
...
- **Total time:** ~3 hours for 6,987 questions (~1.5 hours per model)
...
- Wall time: ~3 hours for complete pipeline (~1.5 hours per model, serial execution)
